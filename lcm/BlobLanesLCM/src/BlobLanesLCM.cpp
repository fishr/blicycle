...
int main(int argc, char** argv) {
	...
	createTrackbar(thresh_label, "gray", &p_trackbar, max_trackbar,
			&blacknwhite);
...
void denoise(int, void*) {
	element = getStructuringElement(MORPH_RECT, Size(xup + 1, yup + 1),
			Point(-1, -1));
	dilate(edges, denoised_img, element);
	element = getStructuringElement(MORPH_RECT, Size(xdown + 1, ydown + 1),
			Point(-1, -1));
...
void showLines(vector<Vec4i> reducedLines, Mat screen) {
	for (size_t i = 0; i < reducedLines.size(); i++) {
		...
		line(screen, Point(l[0], l[1]), Point(l[2], l[3]), Scalar(255, 0, 0), 3,
				CV_AA);
	}
}
...
void findSlope(vector<Vec3f> &laneSlopes) {
	...
	HoughLinesP(denoised_img, p_lines, 1, CV_PI / 180, thresh, minLineLength,
			maxLineGap);
	//}
...
void fitLines(vector<Vec3f> slopes, int numcolumns, vector<int> &low,
		vector<int> &mid, vector<int> &high) {
	...
	for (unsigned int i = 0; i < (high.size()); i++) {
		...
		for (unsigned int x1i = 0; x1i < (mid.size()); x1i++) {
			if (!gotx1) {
				...
				if (fabs(mid.at(x1i) - x1)
						< fabs(lanesX[laneCounter][1] - x1)) { //is this point better than a previous one?
					lanesX[laneCounter][1] = mid.at(x1i);
				}
			}
		}
		...
		for (unsigned int x2i = 0; x2i < (low.size()); x2i++) { //same as above
			if (!gotx2) {
				...
				if (fabs(low.at(x2i) - x2)
						< fabs(lanesX[laneCounter][2] - x2)) {
					lanesX[laneCounter][2] = low.at(x2i);
				}
			}
		}
	...
	for (unsigned int i = 0; i < lanesX.size(); i++) {
		if (lanesX[i][1] != 0) {
			line(lanes, Point(lanesX[i][0], highHeight),
					Point(lanesX[i][1], midHeight), Scalar(0, 0, 255), 10);
		}
		if (lanesX[i][2] != 0) {
			line(lanes, Point(lanesX[i][1], midHeight),
					Point(lanesX[i][2], lowHeight), Scalar(0, 0, 255), 10);
			if ((abs(lanesX[i][2] - xsize / 2) < xdiff)
					&& (lanesX[i][1] != 0)) {
	...
	int nearestLaneLine = nearestElementIndex(stripVectorRow(slopes, 3),
			xsize / 2);
	...
	if ((goodLanes == true) && (lanesX.size() != 0)) {
		...
		nearestTheta = atan2((lowHeight - midHeight),
				((float) (slopes[nearestLaneLine][2]
						- slopes[nearestLaneLine][1])));
		//printf("slopes");
	}
...
int nearestElementIndex(vector<int> searchThis, int findThis) {
	...
	for (unsigned int i = 0; i < searchThis.size(); i++) {
		if (abs(searchThis[i] - findThis)
				< abs(searchThis[bestFit] - findThis)) {
			bestFit = i;
		}
	}
	return bestFit;
